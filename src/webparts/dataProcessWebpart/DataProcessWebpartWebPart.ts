...
import styles from './DataProcessWebpartWebPart.module.scss';
...
export default class DataProcessWebpartWebPart extends BaseClientSideWebPart<IDataProcessWebpartWebPartProps> {
...
<div class="form-group" id='btnfinal'>
  ...
  public render(): void {
    ...
    $('#nonneutralFile,#costFile,#fileShortlist,#newspaperFile').change(function () {
      if ($(this).val()) {
    ...
    sp.web.lists.getByTitle(list.Projects).items.getAll().then((allItems: any[]) => {
      globalData.Projects = allItems;
      for (var index = 0; index < allItems.length; index++) {
        var element = allItems[index];
        $('#projectName').append('<option proj-id="' + element.Id + '" value="' + element.Title + '">' + element.Title + '</option>');
      }
    });

    sp.web.lists.getByTitle(list.ProjectAV).items.getAll().then((allItems: any[]) => {
    ...
    $('#btnSubmit').on('click', function () {
      ...
      if (globalData.QuantityFiles.length == 0) {
        ...
        alertify.set('notifier', 'position', 'top-right');
        alertify.error('Specifications and Quantities must contain atleast one document');
        return;
      }
      if ($("input[name='Specifications']:checked").val() == 'Nonneutral Specifications') {
        if (!globalData.NonneutralSpecificationFile) {
          ...
          alertify.set('notifier', 'position', 'top-right');
          alertify.error('Nonneutral Specifications must contain justification attachment');
          return;
        }
      }
      if (!$("#JOD").val()) {
      ...
      for (let index = 0; index < $('.contact-details').length; index++) {
        ...
        objModel['ContactEmail' + (index + 1)] = $('.contactEmail')[index].value;
        objModel['ContactPhone' + (index + 1)] = $('.contactPhoneNumber')[index].value;
      }
      ...
      sp.web.lists
        ...
        .then((response) => {
          ...
          if (globalData.QuantityFiles) {
            objModel.Specifications = siteURL + '/' + list.SpecificationsandQuantities + '/' + response.data.Id;
          }

          if (globalData.NonneutralSpecificationFile) {
            objModel.justification = siteURL + '/' + list.NonneutralSpecifications + '/' + response.data.Id;
          }

          if (globalData.Estimatedcost) {
            objModel.EstimatedcostDocURL = siteURL + '/' + list.EstimatedcostDocumentLibrary + '/' + response.data.Id;
          }

          if (globalData.ShortlistFile) {
            objModel.ShortlistDocURL = siteURL + '/' + list.ShortlistDocuments + '/' + response.data.Id;
          }

          if (globalData.NewspaperFile) {
            objModel.NewsPaperAdvertisementDocURL = siteURL + '/' + list.NewspaperAdvertisements + '/' + response.data.Id;
          }
          ...
          sp.web.lists
            .getByTitle(list.ProcurementGoods)
            .items.getById(response.data.Id).update(objModel).then(function (result) {
    ...
    function uploadSpecifications(index, id) {
      ...
      sp.web.getFolderByServerRelativePath(list.SpecificationsandQuantities).folders.add(id).then(function (result) {
        result.folder.files.add(file.name, file, true)
          .then(function (result) {
            result.file.listItemAllFields.get().then((listItemAllFields) => {
              sp.web.lists.getByTitle(list.SpecificationsandQuantities).items.getById(listItemAllFields.Id).update({ ProcurementId: id }).then(function (result) {
    ...
    function uploadNonneutralSpecificationFile(id) {
      if (globalData.NonneutralSpecificationFile) {
        commonUpload(list.NonneutralSpecifications, globalData.NonneutralSpecificationFile, id, function () {
    ...
    function uploadEstimatedcostFile(id) {
      if (globalData.Estimatedcost) {
        commonUpload(list.EstimatedcostDocumentLibrary, globalData.Estimatedcost, id, function () {
          uploadShortlist(id);
        });
      } else {
        uploadShortlist(id);
    ...
    function uploadShortlist(id) {
      if (globalData.ShortlistFile) {
        commonUpload(list.ShortlistDocuments, globalData.ShortlistFile, id, function () {
    ...
    function uploadNewspaperFile(id) {
      if (globalData.NewspaperFile) {
        commonUpload(list.NewspaperAdvertisements, globalData.NewspaperFile, id, function () {
          otherFiles(id);
        });
      } else {
        otherFiles(id);
    ...
    function otherFiles(id) { 
      if (globalData.OtherFiles && globalData.OtherFiles.length > 0) {
        ...
        alertify.alert("Data save successfully", function () {
    ...
    function uploadOtherFiles(index, id) {
      ...
      sp.web.getFolderByServerRelativePath(list.GoodsOtherAttachments).folders.add(id).then(function (result) {
        result.folder.files.add(file.name, file, true)
          .then(function (result) {
            result.file.listItemAllFields.get().then((listItemAllFields) => {
              sp.web.lists.getByTitle(list.GoodsOtherAttachments).items.getById(listItemAllFields.Id).update({ ProcurementId: id }).then(function (result) {
    ...
    function commonUpload(listName, file, id, callback) {
      sp.web.getFolderByServerRelativePath(listName).folders.add(id).then(function (result) {
        result.folder.files.add(file.name, file, true)
          .then(function (result) {
            result.file.listItemAllFields.get().then((listItemAllFields) => {
              sp.web.lists.getByTitle(listName).items.getById(listItemAllFields.Id).update({ ProcurementId: id }).then(function (result) {
                callback();
              });
            });
          });
